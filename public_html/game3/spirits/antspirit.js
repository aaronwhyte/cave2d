...
function AntSpirit(screen) {
  ...
  this.type = Game3BaseScreen.SpiritType.ANT;
...
AntSpirit.factory = function(screen, stamp, pos, dir) {
  ...
  b.rad = 0.8;
  b.hitGroup = Game3BaseScreen.Group.ENEMY;
...
AntSpirit.prototype.scan = function(pos, rot, dist, rad) {
  return this.screen.scan(
      Game3BaseScreen.Group.ENEMY_SCAN,
      pos,
      this.scanVec.setXY(
  ...
  if (this.screen.isPlaying()) {
    if (!AntSpirit.OPTIMIZE || this.viewportsFromCamera < AntSpirit.LOW_POWER_VIEWPORTS_AWAY) {
      ...
      if (dist >= 0) {
        ...
        if (otherSpirit && otherSpirit.type == Game3BaseScreen.SpiritType.PLAYER) {
          // attack player!
          this.stress = 0;
...
AntSpirit.prototype.addBullet = function(pos, vel, rad, duration) {
  ...
  b.rad = rad;
  b.hitGroup = Game3BaseScreen.Group.ENEMY_FIRE;
  b.mass = (Math.PI * 4/3) * b.rad * b.rad * b.rad * density;
...
AntSpirit.prototype.explosionSplash = function(pos, rad) {
  ...
  function addSplash(x, y, dx, dy, duration, sizeFactor) {
    s.reset(Game3BaseScreen.SplashType.WALL_DAMAGE, self.stamps.circleStamp);
    s.startTime = now;
    s.duration = duration;
