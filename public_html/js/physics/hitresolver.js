...
HitResolver.prototype.resolveHit = function(e, b0, b1) {
  ...
  var pos = Vec2d.alloc().set(pos1).subtract(pos0);
  var vel = Vec2d.alloc().set(b1.vel).subtract(b0.vel);

  // Calculate accel needed for inelastic resolution.
  // Calc accel along the pos/pos vector by enough to cancel velocity along that direction.
  var accel = Vec2d.alloc().set(vel).projectOnto(pos);
  // Add onto that for elastic collision
  accel.scale(-1 - this.defaultElasticity);
  if (accel.equals(Vec2d.ZERO) || accel.dot(pos) < 0) {
    accel.free();
    pos0.free();
    pos1.free();
    return;
  }
  if (accel.magnitudeSquared() < 0.001) {
    accel.scaleToLength(0.001);
  }

  // Use masses to decide which body gets accelerated how much
  if (b0.mass == Infinity) {
...
//  // TODO no, do it right
//  b0.setVelAtTime(Vec2d.ZERO, e.time);
//  b1.setVelAtTime(Vec2d.ZERO, e.time);
