...
function Editor(playScreen) {
  ...
  this.playScreen.listeners.put(this.gripTrigger);
  ...
  this.bitSize = 0.5;
  this.bitGridMetersPerCell = Editor.BIT_SIZE * BitGrid.BITS;
  this.levelModelMatrix = new Matrix44();
  this.levelColorVector = new Vec4(1, 1, 1);
}
Editor.prototype = new BaseScreen();
Editor.prototype.constructor = Editor;

Editor.BIT_SIZE = 0.5;
Editor.WORLD_CELL_SIZE = Editor.BIT_SIZE * BitGrid.BITS;
...
Editor.prototype.updateSharableUrl = function() {
...
Editor.prototype.onPointerDown = function(pageX, pageY) {
  if (!this.paused) {
    this.controller.requestPointerLock();
  }
};

Editor.prototype.setScreenListening = function(listen) {
...
Editor.prototype.lazyInit = function() {
  if (!this.levelStamps) {
    this.initPermStamps();
  }
  if (!this.world) {
...
Editor.prototype.initPermStamps = function() {
  this.levelStamps = [];
...
Editor.prototype.initWorld = function() {
  this.lastPathRefreshTime = -Infinity;
  var groupCount = Object.keys(Editor.Group).length;
  this.world = new World(Editor.WORLD_CELL_SIZE, groupCount, [
    [Editor.Group.EMPTY, Editor.Group.EMPTY],
    [Editor.Group.ROCK, Editor.Group.WALL],
    [Editor.Group.ROCK, Editor.Group.ROCK],
    [Editor.Group.CURSOR, Editor.Group.WALL],
    [Editor.Group.CURSOR, Editor.Group.ROCK]
  ]);
...
Editor.prototype.toJSON = function() {
  ...
  for (var bodyId in this.world.bodies) {
    ...
    if (body.hitGroup != Editor.Group.WALL) {
...
Editor.prototype.maybeLoadWorldFromFragment = function(frag) {
  try {
    var squisher = new Squisher();
  ...
  if (jsonObj) {
    ...
    for (var i = 0; i < jsonObj.spirits.length; i++) {
      ...
      if (spiritType == Editor.SpiritType.BALL) {
      ...
      } else if (spiritType == Editor.SpiritType.SOUND) {
    ...
    if (jsonObj.splashes) {
      ...
      for (var i = 0; i < jsonObj.splashes.length; i++) {
        ...
        if (splashType == Editor.SplashType.NOTE) {
...
Editor.prototype.createDefaultWorld = function() {
  for (var i = 0; i < 16; i++) {
...
Editor.prototype.initBoulder = function(pos, rad) {
  var density = 1;
  var b = Body.alloc();
  b.shape = Body.Shape.CIRCLE;
  b.setPosAtTime(pos, this.world.now);
  b.rad = rad;
  b.hitGroup = Editor.Group.ROCK;
...
Editor.prototype.initSoundSpirit = function(pos, rad, measureFraction, sixteenth) {
  var density = 1;
  var b = Body.alloc();
  b.shape = Body.Shape.CIRCLE;
  b.setPosAtTime(pos, this.world.now);
...
Editor.prototype.createCursorBody = function() {
  ...
  b.hitGroup = Editor.Group.CURSOR;
  return b;
};

Editor.prototype.initWalls = function() {
...
Editor.prototype.digTerrainAtPos = function(pos) {
  this.bitGrid.drawPill(new Segment(pos, pos), 15, 1);
  this.flushTerrainChanges();
};

Editor.prototype.flushTerrainChanges = function() {
 ...
 * @param cellId
 */
Editor.prototype.changeTerrain = function(cellId) {
...
Editor.prototype.loadCellXY = function(cx, cy) {
...
Editor.prototype.unloadCellXY = function(cx, cy) {
  this.unloadCellId(this.bitGrid.getCellIdAtIndexXY(cx, cy));
};

Editor.prototype.unloadCellId = function(cellId) {
  var tile = this.tiles[cellId];
  if (!tile) return;
  if (tile.stamp) {
 ...
 */
Editor.prototype.createWallBody = function(rect) {
  var b = Body.alloc();
  b.shape = Body.Shape.RECT;
  b.setPosAtTime(rect.pos, this.world.now);
  b.rectRad.set(rect.rad);
  b.hitGroup = Editor.Group.WALL;
  b.mass = Infinity;
  b.pathDurationMax = Infinity;
  return b;
};

Editor.prototype.createTileStamp = function(rects) {
...
Editor.prototype.createWallModel = function(rect) {
...
Editor.prototype.doCursorHoverScan = function() {
  ...
  for (i = 0; i < overlapBodyIds.length; i++) {
    ...
    if (hitBody) {
      if (hitBody.hitGroup == Editor.Group.WALL) {
        overWall = true;
...
Editor.prototype.addNoteSplash = function(x, y, dx, dy, r, g, b, bodyRad) {
  var fullRad = bodyRad * 3;// * (1+Math.random()/2);
  var s = this.splash;
  s.reset(Editor.SplashType.NOTE, this.soundStamp);
...
Editor.prototype.onHitEvent = function(e) {
...
Editor.prototype.updateViewMatrix = function() {
  // scale
...
Editor.prototype.getPauseTriggerColorVector = function() {
  var touchiness = this.touchDetector.getVal();
...
Editor.prototype.unloadLevel = function() {
  if (this.tiles) {
...
Editor.prototype.getBodyPos = function(body) {
  return body.getPosAtTime(this.world.now, this.vec2d);
};

Editor.prototype.showPausedOverlay = function() {
  document.querySelector('#pausedOverlay').style.display = 'block';
};

Editor.prototype.hidePausedOverlay = function() {
