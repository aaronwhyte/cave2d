...
function Test43PlayScreen(controller, canvas, renderer, stamps, sfx) {
  ...
  this.camera = new Camera(0.2, 0.6, Test43PlayScreen.WORLD_RAD * 1.2);
...
Test43PlayScreen.prototype.createDefaultWorld = function() {
  ...
  this.tileGrid.drawTerrainPill(Vec2d.ZERO, Vec2d.ZERO, Test43PlayScreen.WORLD_RAD, 1);
  var ants = 5;
...
Test43PlayScreen.prototype.addAnt = function() {
  this.addItem(Test43BaseScreen.MenuItem.ANT,
      new Vec2d(0, Test43PlayScreen.WORLD_RAD * 0.9 * Math.sqrt(Math.random() * 0.9 + 0.1))
          .rot(2 * Math.PI * Math.random()),
      2 * Math.PI * Math.random());
  this.antCount++;
};

Test43PlayScreen.prototype.removeAnt = function() {
  for (var id in this.world.spirits) {
    var s = this.world.spirits[id];
    if (s.type == Test43BaseScreen.SpiritType.ANT) {
      this.removeByBodyId(s.bodyId);
      this.antCount--;
      break;
    }
  }
...
Test43PlayScreen.prototype.onFrameEnd = function(startTimeMs) {
  var now = performance.now();
  var msTaken = now - startTimeMs;

  // Try to hit a few ms under the time budget.
  this.badness = (this.badness * 0.9) + (msTaken - this.getMsUntilClockAbort() + 4) * 0.1;
  if (this.badness > 1) {
    this.removeAnt();
  } else if (this.badness < -1) {
    this.addAnt();
  }
  if (this.lastCountTimeMs + 1000 < now) {
    // console.log('ant count: ' + this.antCount);
    this.lastCountTimeMs = now;
  }
};
...
Test43PlayScreen.prototype.checkPlayerAntHit = function(pair) {
  // TODO: Some cute sound and splash but no death.
};
